class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        while min_heap:
            ...
            # check if we've seen this (num_stops + stop) before 
            # DON'T transverse down a path if its more expensive than prior 
            if (curr_stop, num_stops) in seen and seen[(curr_stop, num_stops)] <= curr_cost:
                continue 
            
            seen[(curr_stop, num_stops)] = curr_cost
